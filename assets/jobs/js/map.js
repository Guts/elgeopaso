// map.js - accès direct à l'API elgeopasso
$(document).ready(function () {
    ...
    function fetchJobsData() {
        ...
        const externalApiUrl = 'https://elgeopaso.georezo.net/api/offres/?format=json';
    ...
    // Fonction pour compter les offres par département
    // même logique
    function countJobsByDepartment(jobs) {
        var counts = {};

        jobs.forEach(job => {
            //  Le lieu est dans un objet "place"
    ...
    // Fonction pour récupérer les offres d'un département spécifique
    ...
    function getJobUrl(job) {
        // Utilisation de raw_offer comme identifiant
        if (job.raw_offer) {
    ...
    function showJobListPopup(deptCode, deptName, clickLocation) {
        ...
        if (jobCount > 0) {
            departmentJobs.forEach((job, index) => {
                ...
                // Déterminer l'identifiant à afficher
    ...
    // Fonction pour ajouter les CERCLES avec le nombre d'offres
    ...
    map.on('load', function () {
        ...
        fetch('/static/geojson/fr_departements_jobs.geojson')
            ...
            .then(geojsonData => {
                ...
                // Récupérer les données DIRECTEMENT depuis l'API geoelpasso et ajouter les cercles
                // Plus de proxy Django - appel direct à l'API
                fetchJobsData().then(() => {
                ...
                // Fonction pour mettre en évidence le département cliqué

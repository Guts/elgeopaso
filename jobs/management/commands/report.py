...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        base_msg = "BASE GLOBALE DES {} OFFRES\n"\
                   ...
                   "\n=====================================================\n"\
                   .format(offers_all,
                           no_place_all, int(100*no_place_all/offers_all),
                           no_contract_all, int(100*no_contract_all/offers_all),)
        # -- NEW OFFERS
        ...
        if no_place.count():
            edit_url = ["- {}/admin/jobs/georezorss/{}/change/"
                        .format(settings.BASE_URL, i)
                        for i in no_place.values_list("id_rss", flat=True)]
            place_msg = "{} offre(s) sans lieu : \n\t{}"\
                        .format(no_place.count(), "\n\t".join(edit_url))
        else:
            place_msg = "RAS : un lieu a été trouvé pour toutes les"\
        ...
        if no_contract.count():
            ...
            contract_msg = "{} offre(s) sans contrat : \n\t{}"\
                           .format(no_contract.count(), "\n\t".join(edit_url))
        else:
            contract_msg = "RAS : un type de contrat a été trouvé pour"\
                           " toutes les nouvelles offres publiées."
        # DB checks
        ct_broken_places = PlaceVariations.objects\
                                          .exclude(name__in=Place.objects.all())\
                                          .count()
        ct_broken_techs = TechnologyVariations.objects\
                                              .exclude(name__in=Technology.objects.all())\
                                              .count()

        ct_broken_jobs = JobPositionVariations.objects\
                                              .exclude(name__in=JobPosition.objects.all())\
                                              .count()
        ...
        if (ct_broken_places + ct_broken_techs + ct_broken_jobs):
            db_msg = "\n{} relations cassées dans les lieux"\
                     ...
                     "\n{} relations cassées dans les métiers"\
                     .format(ct_broken_places,
                             ct_broken_techs,
                             ct_broken_jobs)
        else:

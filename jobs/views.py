...
def stats_home(request):
    ...
    # KPIs
    nb_contrats = Offer.objects.count()
...
def stats_contrats(request):
    ...
    week_actual_year = Offer.objects.filter(week="{}{}".format(*utc.shift(weeks=-1).isocalendar()[0:2])).count()
    week_last_year = Offer.objects.filter(week="{}{}".format(*utc.shift(weeks=-1, years=-1).isocalendar()[0:2])).count()
    week_comparison_perc = round((week_actual_year-week_last_year)/week_last_year*100, 2)
    print(week_actual_year, week_last_year, week_comparison_perc)
    ...
    { "x": "Apprentissage", "y": Offer.objects.filter(contract="Alternance").count()},
    { "x": "Recherche", "y": Offer.objects.filter(contract="Thèse").count() +  Offer.objects.filter(contract="Postdoctorat").count()},
    { "x": "Autres", "y": Offer.objects.filter(contract="Expatriation").count() + Offer.objects.filter(contract="Autres").count() + Offer.objects.filter(contract="Volontariat").count()},
    { "x": "Non reconnu", "y": Offer.objects.filter(contract="ND").count()},
        ]

    # values to replace within the template
    context = {
        "global_kpi": [nb_offers, nb_contract_perc, nb_place_perc, week_comparison_perc],
...
def get_types_contract_by_period(request):
    ...
    Called via AJAX
    '''
    conts_abbrvs = list(Contract.objects
                        .exclude(abbrv__in=["Autres", "Expatriation", "ND"])
                        .values_list('abbrv', flat=True)
                        .order_by("abbrv"))
    colors = ('#8dd3c7', '#fdb462', '#fb8072', '#80b1d3',
              '#bebada', '#b3de69', '#fccde5', '#d9d9d9',
              '#fccde5', '#bc80bd')
    ...
    if period == 'year':
        ...
        for ct in conts_abbrvs:
            cts_years.append({"color": colors[conts_abbrvs.index(ct)],
                              "key": ct,
                              "values": [(y, Offer.objects
                                                  .filter(contract=ct,
                                                          pub_date__year=y)
                                                  .count()
                                          )
...
def get_contracts_by_technos(request):
    ...
    # logger.debug(softs, cts)

    # variables from models
    years = [i.year for i in Offer.objects.dates('pub_date', 'year')]
    tech_top5 = Offer.objects\
    ...
    for tek in tech_top5:
        ...
        cts_tech.append({"key": tek_name,
                         # "key": "{} ({})".format(tek_name, tek_count),
                         "values": [(y, Offer.objects
                                             .select_related()
                                             .filter(technologies__name=tek_name,
                                                     # contract="CDI",
                                                     pub_date__year=y)
                                             .count())
                                    for y in years]
...
def get_fr_dpts_top10(request):
    ...
    # variables from models
    dpts_toms_top10 = dtps_toms.values('place__name')\
                               .annotate(y=Count('place'))\
                               .order_by('-y')[:10]
    dpts_toms_others = dtps_toms.exclude(place__code__in=dpts_toms_top10.values_list("place__code", flat=True))

    ct_dpts_toms = [{"x": "Autres", "y": dpts_toms_others.count()}, ]
...
def get_countries_top5(request):
    ...
    # variables from models
    countries = Offer.objects.exclude(place__code="FRA")\
                             .filter(place__scale="COUNTRY")
    countries_top5 = countries.values('place__name')\
                              .annotate(y=Count('place'))\
                              .order_by('-y')[:5]
    countries_others = countries.exclude(place__code__in=countries_top5.values_list("place__code", flat=True))

    ct_countries = [{"x": "Autres", "y": countries_others.count()}, ]
...
def timeline(request):
    ...
    dico_styles = {"Alternance": ("far fa-star-half", "primary disabled"),
    ...
    top50 = [{"title": i.title,
              ...
              "link": "https://georezo.net/forum/viewtopic.php?pid={}"
                      .format(i.id_rss),
...
def search(request):
    ...
    offers_filtered = OfferFilter(request.GET,
                                  queryset=offers_qs
                                  )
    ...
    return render(request, 'jobs/search.html',
                  {'offers': offer_filter, "filter": offers_filtered}
                 )
